...
export function WinnerBanner({ data }: Props) {
  ...
  if (scoreA === scoreB) {
    return (
      <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/50 px-5 py-4">
    ...
    >
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          Risk score {winnerScore}/100 — {pct}% lower than {scoreA < scoreB ? postcode_b : postcode_a} ({loserScore}/100)
        </p>
      </div>
    </div>
  )
